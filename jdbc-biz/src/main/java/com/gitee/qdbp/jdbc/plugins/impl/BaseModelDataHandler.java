...
public abstract class BaseModelDataHandler implements ModelDataHandler {
    ...
    protected void fillValueIfAbsent(Map<String, Object> target, String fieldName, Object value, AllFields allFields) {
        if (target == null || VerifyTools.isBlank(fieldName) || VerifyTools.isBlank(value)) {
            return;
        }

        if (allFields.contains(fieldName) && !target.containsKey(fieldName)) {
            target.put(fieldName, value);
        }
    }
    ...
    protected void fillValueIfAbsent(DbWhere where, String fieldName, String tableAlias, Object value,
        ...
        String fullFieldName = DbTools.toFullFieldName(fieldName, tableAlias);
        if (allFields.contains(fullFieldName) && !where.contains(fullFieldName)) {
            where.on(fullFieldName, "=", value);
        }
    }

    protected void fillValueIfAbsent(DbWhere where, String fieldName, Object value, AllFields allFields) {
        if (where == null || VerifyTools.isBlank(fieldName) || VerifyTools.isBlank(value)) {
            return;
        }
        if (allFields.contains(fieldName) && !where.contains(fieldName)) {
            where.on(fieldName, "=", value);
        }
    }

    protected void fillValueIfAbsent(DbUpdate ud, String fieldName, Object value, AllFields allFields) {
        if (ud == null || VerifyTools.isBlank(fieldName) || VerifyTools.isBlank(value)) {
            return;
        }
        if (allFields.contains(fieldName) && !ud.contains(fieldName)) {
