    ...
    public void init() {
        {
            ...
            buffer.ad("NAME VARCHAR(30) NOT NULL COMMENT '名称',");
        ...
        {
            SqlBuilder buffer = new SqlBuilder();
            buffer.ad("CREATE TABLE IF NOT EXISTS TEST_LOGGER (");
            buffer.ad("ID VARCHAR(50) NOT NULL COMMENT '主键',");
            buffer.ad("NAME VARCHAR(20) COMMENT '名称',");
            buffer.ad("CONTENT TEXT NOT NULL COMMENT '内容',");
            buffer.ad("SORT_INDEX INT(8) NOT NULL DEFAULT 1 COMMENT '排序号',");
            buffer.ad("CREATE_TIME DATETIME NOT NULL COMMENT '创建时间',");
            buffer.ad("DATA_STATE INT(10) NOT NULL COMMENT '数据状态:0为正常|其他为删除',");
            buffer.ad("PRIMARY KEY (ID)");
            buffer.ad(")");
        ...
        {
            SqlBuilder buffer = new SqlBuilder();
            buffer.ad("DELETE FROM TEST_LOGGER");
            qdbcBoot.getSqlBufferJdbcOperations().update(buffer.out());
        }
    }
